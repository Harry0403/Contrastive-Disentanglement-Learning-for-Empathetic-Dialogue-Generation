...
if __name__ == "__main__":
    ...
    my_model.encoder.load_state_dict(torch.load("My_model_pth/ceg_dis_encoder.pth",map_location=device))
    my_model.MLP.load_state_dict(torch.load("My_model_pth/ceg_dis_mlp.pth",map_location=device))
    my_model.to(device)
    ...
    with open('discrete_response/ceg_d_EMP.csv', 'w', newline='') as csvfile:
        ...
        for emotion_batch, emo_label, text_batch, llm_text_batch, llm_target_batch, input_batch in bar: 
            ...
            generated_tokens, emo_x = my_model.generate(text_batch)
