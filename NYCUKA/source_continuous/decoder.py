...
from .VAD_encoder_ConClassifier import ContrastiveLearningModel
import torch
import torch.nn as nn
...
class MyModel(nn.Module):
    ...
    def forward(self, text_batch, aug_batch, neg_batch,llm_text_batch, llm_target_batch, emotion_batch=None):
        # encode
        loss_encoder, hidden_states_clean, context_x, emotion_x, predictions_va, emo_va = self.encoder(text_batch, aug_batch, neg_batch, emotion_batch)
        ...
        return loss_encoder, output.loss ,context_x, emotion_x, predictions_va, emo_va
    
    def generate(self, text_batch, emotion_batch):
        hidden_states_clean, pooled_context_x, pooled_emotion_x, predictions_va, emotion_x, emo_va = self.encoder(text_batch,None,None,emotion_batch)
       
        logits = emotion_x @ self.encoder.model.bert.embeddings.word_embeddings.weight.T #(B, n, 32000)
        disentangle_tokens = torch.argmax(logits, dim=-1)#(B,n)
        ...
        return generated_tokens, predictions_va, disentangle_tokens
