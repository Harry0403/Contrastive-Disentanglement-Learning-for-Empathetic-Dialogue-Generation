...
def trainer(dataloader, loader_val, model, epoch=3):
    ...
    for epo in range(epoch):
        ...
        for emotion_batch, text_batch, aug_batch, neg_batch, llm_text_batch, llm_target_batch in bar: ## text positive negative 
            ...
            #trianing step
            optimizer.zero_grad()      
            Infoloss, llm_loss, predictions_va, emo_va = model(text_batch, aug_batch, neg_batch,llm_text_batch, llm_target_batch, emotion_batch)
            ...
            if cnt%20 == 0 :
                if opt_loss > loss:   
                    torch.save(model.encoder.state_dict(), "ceclg_multi_encoder.pth")
                    torch.save(model.MLP.state_dict(), "ceclg_multi_mlp.pth")   
                    opt_loss = loss  
                    print("save")         
                    # break
        # num = -15
...
if __name__ == "__main__":
    ...
    model = MyModel()
    model.encoder.load_state_dict(torch.load("ceclg_d_multi_encoder.pth",map_location=device))
    model.MLP.load_state_dict(torch.load("ceclg_d_multi_mlp.pth",map_location=device))
