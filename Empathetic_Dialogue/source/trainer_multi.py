...
from dataloader_multi import data_loader, predata ,data_loader_val
# from dataloader_multi_d import data_loader, predata ,data_loader_val
from datasets import load_dataset
from decoder_multi import MyModel
import sacrebleu
...
def trainer(dataloader, loader_val, model, epoch=2):
    learning_rate = 1e-5
    ...
    for epo in range(epoch):
        ...
        for emotion_batch, text_batch, prompt_batch, aug_batch, neg_batch,  llm_target_batch in bar: ## text positive negative 
            #chaning device
            ...
            #trianing step
            optimizer.zero_grad()      
            Infoloss, llm_loss, predictions_va, emo_va = model(text_batch, prompt_batch, aug_batch, neg_batch, llm_target_batch, emotion_batch)
            ...
            if cnt%20 == 0 :
                if opt_loss > loss:   
                    torch.save(model.encoder.state_dict(), "v5_ceclg_c_multi_encoder.pth")
                    torch.save(model.MLP.state_dict(), "v5_ceclg_c_multi_mlp.pth")   
                    opt_loss = loss  
                    print("save")         
                    # break
        # num = -15
...
def validation(dataloader, model):
    ...
    with torch.no_grad():
        for emotion_batch, text_batch, llm_target_batch in bar: 
            text_batch = text_batch.to(device)
            emotion_batch = emotion_batch.to(device)
            #llm_target_batch = llm_target_batch.to(device)
            ...
            accuracy=calculate_acc(predictions_va, emo_va)
            acc_my.append(accuracy) 

            # accuracy = calculate_acc_discrete(emo_va, emotion_batch)
            # acc_my.append(accuracy)
            ...
            bar.set_postfix_str(f'rougel:{current_rouge:4f},acc:{accuracy:4f}')
            cnt += 1
            if cnt == 40:                
                break
...
if __name__ == "__main__":
    ...
    loader_val = data_loader_val(b, new_data_val, 1)
    
    model = MyModel()
    model.encoder.load_state_dict(torch.load("v3_ceclg_c_multi_encoder.pth",map_location=device))
    model.MLP.load_state_dict(torch.load("v3_ceclg_c_multi_mlp.pth",map_location=device))
