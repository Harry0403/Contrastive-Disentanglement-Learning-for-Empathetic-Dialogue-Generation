...
def collate_fn(batch, new_data, tokenizer = BertTokenizer.from_pretrained('bert-base-uncased') ,
    ...
    return emotion_batch, text_batch, prompt_batch, aug_batch, aug_batch_neg, llm_target_batch
...
def predata():
    ...
    for ele in tqdm(dataset_val, ncols=0):
        if idx == ele['conv_id']:
            ...
            if prev_conv_id == ele['conv_id'] and prev_context in new_data_val:
                new_data_val[prev_context].pop()
