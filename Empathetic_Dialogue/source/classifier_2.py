...
class BertRegressor(nn.Module):
    ...
    def forward(self, inputs, V_batch=None, A_batch=None ):
        ...
        if V_batch is not None and A_batch is not None :
...
def plot_predictions(valences, arousals, preds_v, preds_a):
    ...
    # True values
    ax.scatter(valences, arousals, color='blue', label='True Values')
    
    # Predictions
    ax.scatter(preds_v, preds_a, color='red', alpha=0.5, label='Predictions')
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), '2_tmpe.bin')
