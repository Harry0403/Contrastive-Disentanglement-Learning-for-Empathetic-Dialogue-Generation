...
def collate_fn(batch, new_data, tokenizer = BertTokenizer.from_pretrained('bert-base-uncased') ,
    ...
    messages = [
        ...
        {"role": "assistant", "content": "I'm afraid I must inform ye that humans cannot eat helicopters."},
    ]    

    ## Tokenizing             
    # emotion_batch = torch.tensor([emotion_to_index[ele['context']] for ele in batch])  #this is for discrete classifer
    # emotion_batch = torch.tensor([ele['context'] for ele in batch]) this is wrong
    ...
    return emotion_label, emo, text_batch,prompt_batch, aug_batch, aug_batch_neg,llm_text_batch, llm_target_batch
...
if __name__ == "__main__": 
    ...
    for emotion_batch, text_batch ,aug_batch, aug_batch_neg, llm_text, llm_target in tqdm(loader): ## text positive negative
        #print(aug_batch.input_ids) #embedding
        break
