...
class ContrastiveLearningModel(nn.Module):
    ...
    def forward(self, input_tokens, input_prompt=None, input_plus_tokens=None, input_minus_tokens=None, emo=None):
        ...
        if input_plus_tokens is not None and input_minus_tokens is not None and emo is not None:
            ...
            # Combine losses
            contrastive_loss = 0.3 * loss_c + loss_e
            ...
            # Total loss
            total_loss = contrastive_loss + 0.8*classifier_loss

            return total_loss, outputs_x.hidden_states[-1], x_emo,0
        else:
            # Inference case
            emotion_logits = self.classifier(pooled_emotion_x)
            x_emo = torch.argmax(emotion_logits, dim=1)
            return outputs_x.hidden_states[-1], x_emo , 0
...
if __name__ == "__main__":
    ...
    cnt = 0
    # Compute loss
    for emo, text_batch, aug_batch, neg_batch, llm_text_batch, llm_target_batch in tqdm(loader): ## text positive negative        
        #print(neg_batch)
        text_batch, aug_batch, neg_batch = text_batch.to(device), aug_batch.to(device), neg_batch.to(device)
        loss, hidden_states_clean, hidden_states_mean = model(text_batch, aug_batch, neg_batch )
        #print(loss, hidden_states_clean.shape)
        cnt += 1
        if cnt == 3 :
            print(loss, hidden_states_clean.shape, hidden_states_mean.shape)
            break