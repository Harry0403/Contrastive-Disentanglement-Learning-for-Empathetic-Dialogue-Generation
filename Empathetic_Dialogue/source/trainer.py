...
def trainer(dataloader, loader_val, model, epoch=2):
    ...
    for epo in range(epoch):
        ...
        for emotion_label, emotion_batch, text_batch, prompt_batch, aug_batch, neg_batch,llm_text_batch, llm_target_batch in bar: ## text positive negative 
            ...
            #trianing step
            optimizer.zero_grad()      
            Infoloss, llm_loss, x_emo = model(text_batch, prompt_batch, aug_batch, neg_batch,llm_text_batch, llm_target_batch, emotion_batch)
            ...
            cnt += 1
            if cnt == 3000 :                
                torch.save(model.encoder.state_dict(), "ceclg_dis_encoder.pth")
                torch.save(model.MLP.state_dict(), "ceclg_dis_mlp.pth")
                # break
        # num = -15
