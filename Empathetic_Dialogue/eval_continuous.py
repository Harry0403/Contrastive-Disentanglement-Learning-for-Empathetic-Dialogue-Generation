...
def plot_predictions(labels, preds_v=None, preds_a=None, preds_d=None, valences=None, arousals=None, dominances=None):
    ...
    plt.savefig(f'ceclg_2000_{xlabel[0]}_{ylabel[0]}.png')
...
if __name__ == "__main__":
    ...
    my_model.encoder.load_state_dict(torch.load("My_model_pth/ceclg_c_v2_encoder.pth",map_location=device))
    my_model.MLP.load_state_dict(torch.load("My_model_pth/ceclg_c_v2_mlp.pth",map_location=device))
    my_model.to(device)
    ...
    loader_val = data_loader_val(b, new_data_val, batch_size=1)
    references = []
    inputs = []
    emotion_labels = []
    prompt_batchs =[]
    ...
    with open('continuous_response/ceclg_conv1_testing_200.csv', 'w', newline='') as csvfile:
        ...
        for prompt_batch, emotion_batch, text_batch, llm_text_batch, llm_target_batch, input_batch, emo_label in bar: 
            ...
            generated_tokens, predictions_va,emo_va = my_model.generate(text_batch, llm_text_batch,emotion_batch)
