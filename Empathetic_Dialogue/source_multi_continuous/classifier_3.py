...
def collate_fn(batch, tokenizer = BertTokenizer.from_pretrained('bert-base-uncased') ):
    ...
    D_batch = torch.tensor([ele['D'] for ele in batch], dtype=torch.float).unsqueeze(1)
    #print(text_batch)

    return text_batch, V_batch, A_batch, D_batch
...
def train_model(model, data_loader, optimizer, device, epoch=3):
    ...
    for text_batch, V_batch, A_batch, D_batch in bar:
        #print(text_batch)
        ...
        loss = model(text_batch,V_batch, A_batch, D_batch)
...
def eval_model(model, data_loader, device):
    ...
    with torch.no_grad():
        ...
        for text_batch, V_batch, A_batch, D_batch in bar:
            ...
            preds_d.extend(predictions[:, 2].cpu().numpy())
            valences.extend(V_batch.cpu().numpy())
            arousals.extend(A_batch.cpu().numpy())
            Dominance.extend(A_batch.cpu().numpy())
    return valences, arousals,Dominance, preds_v, preds_a, preds_d


# Predict and plot results
def plot_predictions(valences, arousals, dominances, preds_v, preds_a, preds_d):
    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # True values
    ax.scatter(valences, arousals, dominances, color='blue', label='True Values')
    
    # Predictions
    ax.scatter(preds_v, preds_a, preds_d, color='red', alpha=0.5, label='Predictions')
    
    ax.set_xlabel('Valence')
    ax.set_ylabel('Arousal')
    ax.set_zlabel('Dominance')
    ax.legend()
    # ax.set_title('True vs Predicted Valence, Arousal, and Dominance')
    plt.savefig('V_A_D.png')
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("3_tmpe.bin",map_location=device))
    ...
    for epoch in range(EPOCHS):
        ...
        valences, arousals, Dominance, preds_v, preds_a, preds_d = eval_model(model, val_data_loader, device)
        print("val")
        
    torch.save(model.state_dict(), '3_tmpe.bin')
    plot_predictions(valences[-10:-1], arousals[-10:-1], Dominance[-10:-1], preds_v[-10:-1], preds_a[-10:-1], preds_d[-10:-1])
    #print(valences, arousals)
    exit()
    ...
    plot_predictions(valences, arousals, preds_v, preds_a)
    print(preds_v, preds_a)