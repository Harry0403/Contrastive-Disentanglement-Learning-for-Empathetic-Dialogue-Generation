...
class ContrastiveLearningModel(nn.Module):
    ...
    def forward(self, input_tokens, input_prompt=None, input_plus_tokens=None, input_minus_tokens=None, emo=None):
        ...
        if input_plus_tokens is not None and input_minus_tokens is not None and emo is not None:
            ...
            # Total loss
            total_loss = 0.8*contrastive_loss + 0.3*classifier_loss
...
if __name__ == "__main__":
    ...
    model.to(device)
    model.classifier.load_state_dict(torch.load("best_model_state.bin",map_location=device))
    cnt = 0
    pre_V_batch = []
    pre_A_batch = []
    V_batch = []
    ...
    # Compute loss
    for emotion_batch, text_batch, aug_batch, aug_batch_neg, llm_target_batch in tqdm(loader): ## text positive negative        
        #print(neg_batch)
        text_batch, aug_batch, aug_batch_neg = text_batch.to(device), aug_batch.to(device), aug_batch_neg.to(device)
        emotion_batch = emotion_batch.to(device)
        total_loss, outputs_x, pooled_context_x, pooled_emotion_x, predictions_va, emo_va = model(text_batch, aug_batch, aug_batch_neg, emotion_batch)
        #print(loss, hidden_states_clean.shape)
        ...
        cnt += 1
        if cnt == 15 :
            plot_predictions(pre_V_batch, pre_A_batch, V_batch, A_batch)
            break
